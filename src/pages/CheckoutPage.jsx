...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      // Sunishchit karein ki script hatate samay woh body mein maujood ho
  ...
  const handlePlaceOrder = async (e) => {
    ...
    const orderData = {
      items: cartItems.map((item) => ({
        ...
        store_id: item.store?.id, // Optional chaining ka istemal karein
      })),
    ...
    try {
      // API call ke liye `body` ko stringify karein
      ...
      if (paymentMethod === "razorpay") {
        ...
        const razorpayOrderResponse = await apiService(endpoints.createRazorpayOrderForMultiple, {
            method: "POST",
        ...
        if (!razorpayKeyId) {
            console.error("CRITICAL: Razorpay Key ID is not defined in .env file.");
